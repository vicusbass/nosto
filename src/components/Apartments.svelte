  ...
  {#each roomOptions as room}
    <button
      ...
      class="min-w-28 font-heading font-light text-base uppercase px-2 md:mx-2 md:px-3 py-1 cursor-pointer border-b-2 border-transparent hover:text-[var(--color-bg-secondary)] hover:border-bg-secondary transition-all
        {selectedRooms.includes(room) ? 'btn-primary' : ''}"
  ...
  <div class="md:col-span-2 md:flex flex-col">
    {#each floors as floor, idx}
      <div class="mb-4">
        <button
          type="button"
          class="w-full px-3 py-4 font-medium text-left rounded-tr-2xl transition-all flex items-center justify-between {selectedIdx ===
          idx
            ? 'bg-main-btn text-white'
            : 'bg-tab-secondary'}"
